export default function PaintEarnSecond() {
  return (
    <div className="relative min-h-screen flex items-center justify-center">
      <div className="ml-[25%] mr-[10%]">
        <div className="mx-16 grid gap-6">
          <div className="grid gap-4">
            <h2 className="text-blue">Rule #3: Get Your Refi Paint!</h2>
            <div>
              <h2 className="text-green">
                Unleash your creativity and paint with confidence using
                sustainable and cost-effective paint.
              </h2>
              <h2>
                We'll provide you what you need, so you can create epic street
                art pieces without harming the environment let's join worlds to
                regenerate into 1
              </h2>
            </div>
          </div>
          <div className="grid gap-4">
            <h2 className="text-blue">
              Rule #4: Level Up with Eclectic Brojects!
            </h2>
            <div>
              <h2 className="text-green">
                Show off your skills and level up with our unique eclectic
                brojects qbits system Engage with the community on sM and be a
              </h2>
              <h2>
                Discord warrior, share your progress using our hashtags, and
                follow our quests & challenges closely You might just get the
                chance to be sent into orbit! Get ready to unleash your creative
                powers and take the street art game to the next level!
              </h2>
            </div>
          </div>
          <div className="grid gap-4">
            <h2 className="text-blue">
              Rule #5: Show Us Your Mural and Crypto in!
            </h2>
            <div>
              <h2 className="text-green">
                Ready to be the next quantum art superstar? Submit your finished
                mural to the community for the ultimate showdown!
              </h2>
              <h2>
                on top of default reward, the community will vote on the best
                mural, with the winner drawing evolving into limited edition
                swag that will make you the talk of the next Web3 irl So, grab
                your spray cans, get those creative juices flowing, and let's
                paint the metaverse up!
              </h2>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
